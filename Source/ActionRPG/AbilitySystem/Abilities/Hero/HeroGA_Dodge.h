...
public:
	/** Ability 激活入口：重置本次闪避的局部时序状态，并驱动完整的闪避起手流程。 */
...
protected:
	...
	/** 真正执行闪避收尾：清理闪避运行态、恢复移动、并在合适时序下交还输入系统。 */
	void FinalizeDodgeAbility();
	/** 根据闪避起手与结束时的移动输入快照，判断是否要在收尾后保留 FastRun。 */
...
protected:
	/** 闪避演出期间附加的临时战斗修正效果。它是资产侧可配置内容，不是运行时自动推导状态。 */
