...
/** 受击后的最终解析结果类型。它表达公共业务结果，不直接绑定某个组件内部阶段。 */
UENUM(BlueprintType)
...
/** 命中希望触发的受击类型。它表达公共受击意图，不等价于最终一定落地的表现结果。 */
UENUM(BlueprintType)
...
public:
	...
	/** 生命伤害的新式等级驱动配置。 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Hit|Damage")
	...
	/** 命中成功后的默认表现配置。 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Hit|Presentation")
...
public:
	/** 生命伤害的新式等级驱动配置。 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "ExecutionHit|Damage")
	FActionAttributeDrivenValueConfig HealthDamageValueConfig;
	...
	/** 命中成功后奖励给攻击方的特殊切武能量。 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "ExecutionHit")
	float SpecialWeaponSwitchEnergyRewardOnHit = 8.f;
	...
	/** 处决命中后默认一定会附带的效果条目。 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "ExecutionHit|Effect")
	TArray<FActionHitEffectEntry> DefaultEffects;
	...
	/** 把处决链共享语义写进一份正式实例载荷，不在这里直接触发命中解析。 */
...
public:
	...
	/** 只判断这次实例是否携带了正式效果请求，不代表后续一定成功应用。 */
 ...
 * 3. 不应再靠推测输入载荷去反推最终结果。
 * 它是最终解析结果层，不再回头承载静态配置或运行时 payload 语义。
 */
USTRUCT(BlueprintType)
