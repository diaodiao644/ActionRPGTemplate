 ...
 * 英雄负载配置资产。
 * 当前职责分为两部分：
 * 1. 提供角色进入游戏时就应存在的常驻 / 事件驱动 / 通用输入能力；
 * 2. 提供固定武器槽的默认武器配置，以及每个槽位自己的主攻击能力与额外战斗能力。
 *
 * 这样处理后：
 * 1. 通用能力仍然从统一数据资产初始化；
...
public:
	...
	/** 进入游戏后立刻授予，且通常会按自身策略自动触发的能力。*/
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "LoadoutData|Ability", meta = (ToolTip = "角色进入游戏后立刻授予的通用能力。适合放授予后自动生效、或不依赖武器槽的常驻能力。"))
	TArray<FActionAbilitySet> ActivateOnGivenAbilities;

	/** 用来响应受击、状态事件等战斗事件的通用能力。*/
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "LoadoutData|Ability", meta = (ToolTip = "用于响应受击、打断、状态变化等战斗事件的通用能力。不要把固定武器槽主攻击能力放在这里。"))
	 ...
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "LoadoutData|Ability", meta = (ToolTip = "与武器槽无关、但依旧由输入驱动的常驻能力。适合放处决、切武请求这类全局输入能力。"))
	 ...
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "LoadoutData|Weapon", meta = (EditFixedSize, ToolTip = "固定四槽配置。顺序始终为：空手、近战、远程、混合。只修改槽内数据，不要尝试把它当成可增删的背包列表。"))
...
public:
	...
	/** 构建当前负载资产的编辑器校验报告。 */
	UFUNCTION(BlueprintPure, Category = "EditorTools|Validation", meta = (DisplayName = "构建负载资产校验报告", ToolTip = "返回当前 HeroLoadoutData 的详细校验结果字符串。适合在编辑器工具蓝图或自定义面板中展示。"))
	FString BuildLoadoutValidationReport() const;

	/** 在日志中输出当前负载资产的编辑器校验报告。 */
	UFUNCTION(CallInEditor, BlueprintCallable, Category = "EditorTools|Validation", meta = (DisplayName = "校验并输出负载资产报告", ToolTip = "检查固定四槽顺序、主攻击能力、额外能力和默认武器定义，并把结果输出到日志。推荐在每次修改后点一次。"))
	void ValidateAndLogLoadoutData() const;

	// 内部标准化构建：
	// 运行时与编辑器工具最终都依赖这一个统一出口来产出标准四槽数组。
	/** 构建一份规范化后的固定武器槽配置，避免槽位顺序失序或缺项。*/
